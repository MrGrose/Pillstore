...
class CRUDBase(Generic[ModelType]):
    ...
    async def get_by_id(self, id: int):
        db_result = await self.session.scalars(select(self.model).where(self.model.id == id))
        result = db_result.first()
        return result
        
    
    async def get_all(self, flag):
        db_result_active = await self.session.scalars(select(self.model).where(self.model.is_active == flag))
