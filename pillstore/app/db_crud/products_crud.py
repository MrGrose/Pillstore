...
class CrudProduct(CRUDBase):
    ...
    async def pagination_page_products(
        ...
        page_urls = await self.generate_page_urls(page_size, request, total_pages, page, search, category_id)
        page_size_urls = await self.generate_page_size_urls(request, search, category_id)
